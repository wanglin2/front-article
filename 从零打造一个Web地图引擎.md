...
那么大家有没有想过这些地图是怎么渲染出来的呢，为什么根据一个经纬度就能显示对应的地图呢，不知道没关系，本文会从零实现一个简单的地图加载器，来帮助大家了解`GIS`基础知识及`Web`地图的实现原理。
...
>谷歌XYZ规范：谷歌地图、OpenStreetMap、高德地图、geoq等，坐标原点在左上角
>
>WMTS标准：腾讯地图、天地图等，坐标原点在左下角
>
>百度地图比较特立独行，啥都跟其他厂商不一样，缩放层级Z从1开始（其他都从0开始），在最高级就把地图分为四块瓦片，XY的原点在经纬度都为0的位置，向右为X正方向，向上为Y正方向

谷歌和`WMTS`的瓦片区别可以通过该地址可视化的查看：[地图瓦片](https://www.maptiler.com/google-maps-coordinates-tile-bounds-projection/#1/25.12/-0.61)。

虽然规范不同，但原理是一致的，都是把地球投影成一个巨大的正方形世界平面图，然后按照四叉树进行分层切割，比如第一层，只有一张瓦片，显示这个世界的信息，所以基本只能看到海和一些边界线，第二层，切割成四张瓦片，显示信息稍微多了一点，以此类推，就像一个金字塔一样，底层分辨率最高，显示的细节最多，瓦片数也最多，顶层分辨率最低，显示的信息很少，瓦片数量相对也最少：
...
上一节里我们简单介绍了一下坐标系，按照`Web`地图的事实标准，我们的地图加载器也选择支持`EPSG:3857`投影，但是我们通过高德工具获取到的是火星坐标系的经纬度坐标，所以第一步要把经纬度坐标转换为`Web墨卡托`投影坐标，这里我们为了简单，先直接把火星坐标当做`WGS-84`坐标，后面再来看这个问题。
...
`3857`坐标有了，它的单位是`米`，那么怎么转换成瓦片的行列号呢，这就涉及到`分辨率`的概念了，即地图上一像素代表实际多少米，分辨率可以这么计算，我们知道地球半径是`6378137`米，`3857`坐标系把地球当做正圆球体来处理，投影贴着地球赤道：
...
![image-20220105150159713.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/125aec7dce294fbc98c4d2c4f96ef7cf~tplv-k3u1fbpfcp-watermark.image?)
...
中心坐标对应的瓦片行列号知道了，那么该瓦片左上角对应的在世界平面图中的像素位置我们也就知道了：
...
中心经纬度的瓦片我们计算出来了，瓦片左上角的像素坐标也知道了，然后我们再计算出中心经纬度本身对应的像素坐标，那么和瓦片左上角的差值就可以计算出来，最后我们把画布的原点移动到画布中间（默认为左上角），那么中心瓦片的显示位置就是这个差值。
...
所以我们只要计算出中心瓦片四个方向各需要几块瓦片，然后用一个双重循环即可计算出画布需要的所有瓦片，计算瓦片数量很简单，请看下图：
...
```js
for (let i = -rowMinNum; i <= rowMaxNum; i++) {// 和中心瓦片的水平距离
    for (let j = -colMinNum; j <= colMaxNum; j++) {// 和中心瓦片的垂直距离
        // 加载瓦片图片
        let img = new Image()
        img.src = getTileUrl(
            centerTile[0] + i,// 行号
...
功能是有了，不过效果很一般，因为我们平常使用的地图缩放都是有一个放大或缩小的过渡动画，而这个是直接空白然后重新显示，不仔细看都不知道是放大还是缩小。
...
前面还遗留了一个问题，即我们把高德工具上选出的经纬度直接当做`4326`经纬度，前面也讲过，它们之间是存在偏移的，比如手机`GPS`获取到的经纬度一般都是`84`坐标，直接在高德地图显示，会发现和你实际位置不一样，所以就需要进行一个转换，有一些工具可以帮你做些事情，比如[Gcoord](https://github.com/hujiulong/gcoord)、[coordtransform](https://github.com/wandergis/coordtransform)等。



# 总结

目前一般都是使用`canvas`来渲染`2D`地图，因为笔者的`canvas`水平限制，所以最终呈现效果比较一般，其实只要稍微加一点瓦片的淡出动画，效果就会好很多，所幸目前有一些强大的`canvas`库可以选择，所以笔者最后又使用[Konva.js](https://konvajs.org/)库来重做了一版，加入了瓦片淡出动画，最终效果如下：
